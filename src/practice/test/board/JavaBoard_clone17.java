...
public class JavaBoard_clone17 {
	...
	public static void main(String[] args) {
		...
		JavaBoard_clone17 jc = new JavaBoard_clone17();
		jc.seeBoard();
	}
	...
	private void seeBoard() {
		...
		while(true) {
			...
			case 1: //조회
				...
				break;
			case 2: //등록
				System.out.println("작성하신 글을 등록합니다.");
				putData(number);
				list.add(data);
				number++;
				break;
	...
	private void seeDetail(int input1) {
		System.out.println(input1+"번 글을 조회합니다.");
	...
	private void modifyDeleteList(int input1) {
		...
		switch (input) {
		case 1:
			System.out.println(input1+"번 글을 수정합니다.");
			for (int i = 0; i < list.size(); i++) {
				if((int)list.get(i).get("번호")==input1) {
		...
		case 2:
			...
			System.out.println(input1+"번 글이 삭제되었습니다.");
			break;
		case 3:
			return;
